defmodule Tackle.Web.ChatStoreTest do
  ...
  describe "creating a conversation" do
    test "records the directory, the model and an empty transcript", %{cwd: cwd} do
      assert {:ok, conversation} = ChatStore.create(cwd: cwd, model: "fake/echo")
    ...
    test "expands a relative directory", %{cwd: cwd} do
      ...
      assert {:ok, conversation} = ChatStore.create(cwd: relative, model: "fake/echo")
      assert conversation.cwd == cwd
    end
    ...
    test "refuses a directory that is not there", %{root: root} do
      ...
      assert {:error, {:workspace_missing, ^missing}} =
               ChatStore.create(cwd: missing, model: "fake/echo")
    end

    test "refuses anything that is not a directory path" do
      assert {:error, {:invalid_workspace, nil}} = ChatStore.create(model: "fake/echo")
      assert {:error, {:invalid_workspace, 42}} = ChatStore.create(cwd: 42, model: "fake/echo")
    end
  end
  ...
  describe "listing" do
    ...
    test "puts the most recently written conversation first", %{cwd: cwd} do
      {:ok, first} = ChatStore.create(cwd: cwd, model: "fake/echo")
      {:ok, second} = ChatStore.create(cwd: cwd, model: "fake/echo")
      ...
      ids = Enum.map(ChatStore.list(), & &1.id)
  ...
  describe "deleting" do
    test "forgets the conversation", %{cwd: cwd} do
      ...
      refute Enum.any?(ChatStore.list(), &(&1.id == conversation.id))
    end
  ...
  defp summary_of(id), do: Enum.find(ChatStore.list(), &(&1.id == id))
