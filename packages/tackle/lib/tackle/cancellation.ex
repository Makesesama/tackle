defmodule Tackle.Cancellation do
  @moduledoc """
  Cooperative cancellation primitives for Tackle runs.

  Tackle runs are synchronous from the library's point of view, but hosts often
  ...
  * tools receive the signal in their context and can stop long-running work.
  ...
  If an adapter or tool ignores the signal, Tackle cannot preempt it mid-call,
  but the loop will settle as cancelled at the next boundary.

  ## Storage

  Cancellation storage is configurable. Tackle defaults to
  `Tackle.Cancellation.Store.Ets` to preserve historical cross-process behavior,
  but hosts can provide any module implementing `Tackle.Cancellation.Store`:

      config :tackle, cancellation_store: MyApp.TackleCancellationStore
  ...
  defmodule Signal do
    @moduledoc """
    Opaque cancellation handle shared between a host and a Tackle run.
    """
  ...
  @default_store Tackle.Cancellation.Store.Ets
  ...
  defp store do
    Application.get_env(:tackle, :cancellation_store) ||
      get_in(Application.get_env(:my_app, Tackle, []), [:cancellation_store]) ||
      @default_store
  end
end
