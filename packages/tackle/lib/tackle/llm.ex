defmodule Tackle.LLM do
  @moduledoc """
  Provider-agnostic LLM behaviour — the keystone seam of Tackle.

  Tackle never talks to a provider directly. The agent loop
  (`Tackle.Loop`) calls `generate/2` on the configured adapter module, and the
  host application supplies an adapter that fulfils this contract. This is what
  makes Tackle provider-agnostic: swap the adapter, keep the loop.

  ## Configuring the adapter

  The adapter is resolved from application config. When Tackle ships as a
  standalone library it reads its own `:tackle` key; while it lives in-tree it
  ...
  separate OTP app just for config:
      ...
      config :my_app, Tackle, llm: MyApp.AI.TackleAdapter

  `adapter/0` reads these (preferring `:tackle`). `generate/2` is a convenience
  that delegates to the configured adapter, so loop code can call
  `Tackle.LLM.generate/2` directly. Adapters may also implement the optional
  `stream/3` callback; Tackle normalizes adapter/provider stream events into
  `%Tackle.Event{}` before hosts see them.
  ...
  * `schema` — an optional Tackle/`AI.Object`-style keyword schema describing a
    structured non-tool response (see `Tackle.SystemPrompt.response_schema/0`),
    or `nil` for a plain/native turn. Tool calls are delivered through
    provider-native `tool_calls`, not through this schema.
  * `opts` — passthrough options. Tackle sets `:model`, `:system`,
  ...
  provider-neutral, role-tagged array (see `Tackle.Messages.to_provider/1`)
  ...
  On success the adapter returns `{:ok, %{data: map, usage: map | Tackle.Usage.t() | nil, model: String.t() | nil}}`
  ...
  provider-specific usage maps into `%Tackle.Usage{}` before returning so the
  ...
  Raises if no adapter is configured — Tackle is provider-agnostic by design and
  has no built-in default; the host app MUST supply one.
  """
  @spec adapter() :: module()
  def adapter do
    resolved =
      Application.get_env(:tackle, :llm) ||
        get_in(Application.get_env(:my_app, Tackle, []), [:llm])

    case resolved do
      nil ->
        raise """
        No Tackle LLM adapter configured. Set one in config:

            config :tackle, llm: MyApp.AI.TackleAdapter

        The adapter module must implement the Tackle.LLM behaviour.
        """
  ...
  receives. Tackle converts each one to `%Tackle.Event{}` before invoking the
  host callback. If the adapter does not implement `stream/3`, Tackle falls back
