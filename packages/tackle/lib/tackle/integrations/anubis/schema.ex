defmodule Tackle.Integrations.Anubis.Schema do
  @moduledoc """
  Peri projection for registering Tackle tools with Anubis.

  Kept in the Anubis integration so `Tackle.Tool.Schema` remains independent of
  any provider/runtime schema format.
  """

  @doc "Projects Tackle's keyword schema into the Peri-style schema map Anubis expects."
  @spec to_peri(Tackle.Tool.Schema.t() | map() | nil) :: map() | nil
  ...
  # boundary; Tackle.Integrations.Anubis decodes string values with the
  # configured Tackle.JSON behaviour before core Tackle validation runs.
