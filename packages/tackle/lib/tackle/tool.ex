defmodule Tackle.Tool do
  @moduledoc """
  Behaviour and dispatch glue for Tackle agent tools.

  A tool is any module the agent loop can describe to the LLM and execute. This
  is the single, clean tool contract Tackle's loop knows about — host
  ...
  The schema itself is owned by `Tackle.Tool.Schema`, not by any provider
  adapter. Provider-specific projections should be derived at the adapter
  boundary (for example, `Tackle.Tool.Schema.JsonSchema.to_json_schema/1`).

  ## Adapting richer tool systems

  Tackle deliberately keeps this contract small. If your app has a more
  elaborate tool abstraction (e.g. surface-agnostic tools shared with an MCP
  server), implement `Tackle.Tool` as a thin façade over it — the loop only ever
  sees this behaviour.
  """
  ...
  """
  @callback parameters_schema() :: Tackle.Tool.Schema.t()

  @doc "Optional provider-neutral output schema for hosts that want to validate results."
  @callback output_schema() :: Tackle.Tool.Schema.output_schema()
  ...
  are owned by the host; Tackle stays agnostic. `description/0` remains the flat
  ...
  @doc """
  Defines a Tackle tool with the canonical DSL.
  ...
  @doc """
  Returns Tackle's provider-neutral definition for a tool module.
  """
  @spec definition(module()) :: %{
          name: String.t(),
          description: String.t(),
          input_schema: [map()],
          output_schema: Tackle.Tool.Schema.output_schema(),
          definition_id: String.t()
        }
  def definition(tool_module) do
    definition =
      Tackle.Tool.Schema.definition(
        tool_module.name(),
  ...
  """
  @spec output_schema(module()) :: Tackle.Tool.Schema.output_schema()
  ...
  The metadata shape is host-defined — Tackle only knows it may exist and that a
  ...
  by the host; Tackle only wires the selection.
  """
  ...
  the Tackle JSON boundary using the configured `Tackle.JSON` adapter.
  """
  ...
  def validate_args(tool_module, args) do
    ...
    case Tackle.Tool.Schema.validate(schema, normalize_map_arguments(schema, args || %{})) do
      {:ok, normalized} -> {:ok, normalized}
      {:error, errors} -> {:error, Tackle.Tool.Schema.format_errors(errors)}
    end
  end
  ...
  """
  @spec settle(module(), Tackle.Tool.Call.t(), map()) ::
          {:ok, Tackle.Tool.Result.t()} | {:error, Tackle.Tool.Error.t()}
  def settle(tool_module, %Tackle.Tool.Call{} = call, context) do
  ...
  def execute_tool_call(tool_module, args, context) do
    call = %Tackle.Tool.Call{id: "legacy", name: tool_module.name(), arguments: args || %{}}

    case settle(tool_module, call, context) do
      {:ok, %Tackle.Tool.Result{content: content}} ->
        {:ok, content}

      {:error, %Tackle.Tool.Error{reason: :execution_error, message: message}} ->
        {:error, message}

      {:error, %Tackle.Tool.Error{content: content}} ->
        {:error, content}
    end
  end
  ...
  defp validate_output(tool_module, result) do
    case output_schema(tool_module) do
      ...
      schema ->
        case Tackle.Tool.Schema.validate_output(schema, result) do
          {:ok, output} -> {:ok, output}
  ...
  defp normalize_map_argument(args, field, :map) do
    case fetch_argument(args, field) do
      {:ok, key, value} when is_binary(value) ->
        case Tackle.JSON.decode(value) do
