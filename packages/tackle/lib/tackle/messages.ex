defmodule Tackle.Messages do
  @moduledoc """
  Converts a Tackle conversation (a list of `%Tackle.Message{}`) into a
  provider-neutral **structured message array** — the shape mature agent
  harnesses send to chat-completion APIs.

  Tackle's loop keeps the conversation as typed messages (user / assistant /
  ...
  @doc """
  Converts a list of `%Tackle.Message{}` into provider-neutral structured maps.
  ...
  @doc """
  Converts a single `%Tackle.Message{}` into a provider-neutral structured map,
  ...
  defp tool_call_id(_), do: Tackle.ID.uuid4()
  ...
  defp encode_arguments(args) when is_map(args), do: Tackle.JSON.encode!(args)
