defmodule Tackle.ToolTest do
  use ExUnit.Case, async: true

  alias Tackle.Tool
  alias Tackle.Tool.Schema
  alias Tackle.Tool.Schema.JsonSchema
  ...
  test "use Tackle.Tool defines the canonical callback API" do
  ...
  test "use Tackle.Tool defaults output_schema/0 to nil when no output block is declared" do
    assert DSLNoOutputTool.output_schema() == nil
    assert {:ok, %{"value" => "hello"}} = DSLNoOutputTool.execute(%{"value" => "hello"}, %{})
  end

  test "use Tackle.Tool requires run/2 at compile time" do
    assert_raise ArgumentError, ~r/must define run\/2/, fn ->
      Code.compile_string("""
      defmodule Tackle.ToolTest.MissingRunTool do
        use Tackle.Tool
  ...
  test "field/3 must be declared inside an input or output block" do
    assert_raise ArgumentError, ~r/field\/3 must be called inside an input or output block/, fn ->
      Code.compile_string("""
      defmodule Tackle.ToolTest.StrayFieldTool do
        use Tackle.Tool
  ...
  test "DSL tools are accepted by the registry" do
    registry = Tackle.Tool.Registry.new([DSLSearchTool])

    assert [%{name: "search", output_schema: [results: [type: {:list, :map}, required: true]]}] =
             Tackle.Tool.Registry.definitions(registry)
  end

  test "settle/3 returns structured successful results" do
    call = %Tackle.Tool.Call{
      id: "call_1",
      name: "echo",
      arguments: %{"query" => "hi", "limit" => "3"}
    }

    assert {:ok, %Tackle.Tool.Result{} = result} = Tool.settle(EchoTool, call, %{})
    assert result.tool_call_id == "call_1"
    assert result.output == %{"limit" => 3, "query" => "hi"}
    assert {:ok, %{"limit" => 3, "query" => "hi"}} = Tackle.JSON.decode(result.content)
  end

  test "settle/3 returns structured output validation errors" do
    call = %Tackle.Tool.Call{id: "call_2", name: "invalid_output", arguments: %{}}

    assert {:error, %Tackle.Tool.Error{} = error} = Tool.settle(InvalidOutputTool, call, %{})
  ...
  test "execute_tool_call/3 validates before executing" do
    ...
    assert {:ok, %{"limit" => 3, "query" => "hi"}} = Tackle.JSON.decode(encoded)
  ...
  test "registry assigns stable definition ids and rejects stale calls" do
    registry = Tackle.Tool.Registry.new([EchoTool])
    [definition] = Tackle.Tool.Registry.definitions(registry)

    assert %{definition_id: definition_id, name: "echo"} = definition

    fresh_call = %Tackle.Tool.Call{name: "echo", definition_id: definition_id}
    stale_call = %Tackle.Tool.Call{name: "echo", definition_id: "old"}

    assert {:ok, %{module: EchoTool}} = Tackle.Tool.Registry.resolve(registry, fresh_call)
    assert {:error, :stale_tool_definition} = Tackle.Tool.Registry.resolve(registry, stale_call)
  end
