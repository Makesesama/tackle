defmodule Tackle.LLMTest do
  ...
  setup do
    previous = Application.get_env(:tackle, :llm)
    Application.put_env(:tackle, :llm, TestAdapter)

    on_exit(fn ->
      if previous do
        Application.put_env(:tackle, :llm, previous)
      else
  ...
  test "normalizes adapter stream events into Tackle events" do
    test_pid = self()
