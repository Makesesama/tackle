defmodule Tackle.LoopTest do
  ...
  setup do
    previous_llm = Application.get_env(:tackle, :llm)
    previous_instruction = Application.get_env(:tackle, :instruction)
    Application.put_env(:tackle, :llm, TestAdapter)

    on_exit(fn ->
      if previous_llm do
        Application.put_env(:tackle, :llm, previous_llm)
      else
        Application.delete_env(:tackle, :llm)
      end

      if previous_instruction do
        Application.put_env(:tackle, :instruction, previous_instruction)
      else
        Application.delete_env(:tackle, :instruction)
      end
    end)
  end

  test "uses configured loop instruction" do
    Application.put_env(:tackle, :llm, MessagesCapturingAdapter)

    Application.put_env(:tackle, :instruction,
  ...
  test "native protocol: plain content is treated as the final answer" do
    Application.put_env(:tackle, :llm, NativeContentAdapter)
    Process.put(:test_pid, self())
  ...
  test "native protocol: native tool_calls drive tool execution then plain-content answer" do
    Application.put_env(:tackle, :llm, NativeToolThenContentAdapter)
    Process.put(:test_pid, self())
    Process.put(:call_count, 0)
  ...
  test "passes a structured message array (not a flat blob) with linked tool call/result" do
    Application.put_env(:tackle, :llm, StructuredMessagesCapturingAdapter)
    Process.put(:test_pid, self())
    Process.put(:call_count, 0)
  ...
  test "can route provider streaming through Tackle event deltas" do
    test_pid = self()
    state = State.new(model: "test/model")
  ...
  test "executes multiple tool calls sequentially and strips unsupported tool policy opts" do
    Application.put_env(:tackle, :llm, ToolCallingAdapter)
    Process.put(:test_pid, self())
    Process.put(:call_count, 0)
  ...
  test "cancels before starting the LLM call" do
    Application.put_env(:tackle, :llm, NoCallAdapter)
    Process.put(:test_pid, self())
  ...
  test "cancels between sequential tool calls and exposes the signal to tools and adapters" do
    Application.put_env(:tackle, :llm, CancellingToolAdapter)
    Process.put(:test_pid, self())
    Process.put(:call_count, 0)
  ...
  describe "hook integration" do
    ...
    defmodule SwapAdapterConfigHook do
      ...
      @impl true
      def before_prompt(_state, context) do
        Application.put_env(:tackle, :llm, ReplacementAdapter)
    ...
    test "lifecycle hooks fire in correct order during a turn" do
      Application.put_env(:tackle, :llm, HookTestAdapter)
    ...
    test "snapshot freezes the LLM adapter for the turn" do
      Application.put_env(:tackle, :llm, SnapshotAdapter)
      Process.put(:test_pid, self())
