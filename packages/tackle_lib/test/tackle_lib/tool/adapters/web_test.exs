defmodule Tackle.Lib.Tool.Adapters.WebTest do
  use ExUnit.Case, async: true

  alias Tackle.Lib.Tool.Adapters.Web
  ...
  test "wrap/1 accepts manually implemented Tackle.Lib.Tool modules" do
    assert Web.wrap([ManualTool]) == [ManualTool]
  end

  test "wrapped tools can be passed to Tackle.Lib.Tool.Registry" do
    registry = Web.wrap([SearchTool]) |> Tackle.Lib.Tool.Registry.new()

    assert [%{name: "search"}] = Tackle.Lib.Tool.Registry.definitions(registry)
  end
