defmodule Tackle.Lib.JSON do
  @moduledoc """
  JSON boundary used by Tackle.Lib.

  Tackle.Lib is intended to be embedded in many Elixir applications, so it does not
  ...
  `Tackle.Lib.JSON.Default` is used, which delegates to Elixir's built-in `JSON`
  module.
  """
